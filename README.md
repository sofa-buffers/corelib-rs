...
[`generator`](https://github.com/sofa-buffers/generator) turns a schema into typed
structs with `encode()` / `decode()` (over `marshal` / a `Visitor`). A hand-written
stand-in, encoded then decoded:

```rust
use sofab::{OStream, IStream, Visitor, Id, Signed};

// generated by: sofabgen --lang rust
#[derive(Default)]
...
impl Point {
    ...
    fn decode(data: &[u8]) -> Self {
        ...
        IStream::new().feed(data, &mut m).unwrap();
        m
    }
}
...
let got = Point::decode(&wire);            // got.x == 3, got.y == 4
```
