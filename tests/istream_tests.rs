...
#[test]
fn id_above_max_is_invalid_inside_a_sequence_too() {
    // The ceiling binds a value-bearing header wherever it sits — nesting it in
    // an open (and, to a schema, unknown) sequence must not smuggle it past.
    let header = (sofab::ID_MAX as u64 + 1) << 3; // type tag 0 = unsigned
    let mut bytes = vec![0x76]; // sequence start, id 14
    push_varint(&mut bytes, header);
    bytes.push(0x00); // value
    bytes.push(0x07); // sequence end
    let mut rec = Recorder::new();
    let mut is = IStream::new();
    assert_eq!(is.feed(&bytes, &mut rec), Err(Error::InvalidMsg));
}
...
#[test]
fn sequence_end_id_is_discarded_whatever_its_value() {
    // §4.9: a sequence-end header carrying *any* id decodes as an ordinary
    // sequence end, its id normalized away. §6.2 scopes ID_MAX to value-bearing
    // headers, so an id past the ceiling is accepted here and only here.
    //
    // Includes the F-0054 isolate (`76 87 80 80 80 40`): id 2³¹, one past
    // ID_MAX, closing a sequence opened under an undeclared id.
    let ids = [
        0,                        // canonical `0x07`
        3,                        // small non-zero
        sofab::ID_MAX as u64,     // at the ceiling
        sofab::ID_MAX as u64 + 1, // one past it — the isolate
        u64::MAX >> 3,            // the largest a 64-bit header varint can express
    ];
    for id in ids {
        let mut bytes = vec![0x76]; // sequence start, id 14
        push_varint(&mut bytes, (id << 3) | 0x07);
        let mut rec = Recorder::new();
        let mut is = IStream::new();
        assert_eq!(
            is.feed(&bytes, &mut rec),
            Ok(()),
            "sequence end with id {id} rejected"
        );
        assert_eq!(rec.events, [Event::SequenceBegin(14), Event::SequenceEnd]);
    }

    // The isolate verbatim, as the six bytes a sender puts on the wire.
    assert_eq!(
        decode(&[0x76, 0x87, 0x80, 0x80, 0x80, 0x40]),
        [Event::SequenceBegin(14), Event::SequenceEnd]
    );
}

#[test]
fn a_sequence_end_header_is_still_a_bounded_varint() {
    // What §4.9 drops is the ID_MAX *value* bound, not §4.1's bound on the
    // encoding: an 11-byte header varint stays INVALID on a sequence end just
    // as anywhere else.
    let mut bytes = vec![0x76]; // sequence start, id 14
...
#[test]
fn a_sequence_end_with_an_oversized_id_still_needs_an_open_sequence() {
    // Discarding the id does not make the marker balanced: with no sequence
    // open it is INVALID, exactly as the bare `0x07` is (§5.2).
    let mut bytes = Vec::new();
